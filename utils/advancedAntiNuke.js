...
class AdvancedAntiNuke {
      ...
      if (permCheck.suspicious) {
        ...
        if (permCheck.confidence >= 60) {
          try {
            ...
            if (!member) {
              return;
            } // Member already banned/kicked - stop processing
  ...
  async handleThreat(guild, userId, threatType, counts) {
    ...
    if (isOwner) {
      ...
      // Admin threat detected - proceeding with action
    }
    ...
    if (!actionTaken) {
      // Immediate ban/kick failed - attempting role manipulation fallback
    }
    ...
    try {
        ...
        try {
          ...
          for (const role of allRolesToRemove.values()) {
            try {
              ...
              // Could not remove role - continue with next role
            }
          }
      ...
      // Check current role positions
      if (botMemberCheck && member.roles.highest) {
        // Bot and attacker role positions checked
      }
      ...
      // Check if admin is being banned
      const isAdmin = member.permissions.has("Administrator");
      ...
      if (!removed && member.id !== guild.ownerId) {
        ...
        if (!removed) {
          // All removal methods failed - attempting role removal
          if (botMemberCheck && member.roles.highest) {
