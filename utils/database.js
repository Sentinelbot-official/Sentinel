...
class Database {
        ...
        if (err) {
          ...
          this.db.serialize(() => {
            ...
            this.db.run("PRAGMA journal_mode = WAL;", (err) => {
              if (err) {
                logger.warn("Failed to enable WAL mode:", err);
              }
            });
  ...
  initTables() {
    ...
    // API key generation requests (pending confirmation)
    this.db.run(`
    ...
    this.db.run(`
            CREATE INDEX IF NOT EXISTS idx_api_key_requests_user 
            ON api_key_requests(discord_user_id, status)
        `);

    // Comprehensive endpoint access logs (all endpoints - admin, user, API)
    this.db.run(`
            CREATE TABLE IF NOT EXISTS endpoint_access_logs (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                ...
                response_status INTEGER,
                api_key TEXT,
                admin_authenticated INTEGER DEFAULT 0,
                timestamp INTEGER,
                error_message TEXT
            )
        `);
    ...
    this.db.run(`
            CREATE INDEX IF NOT EXISTS idx_endpoint_access_user 
            ON endpoint_access_logs(discord_user_id, timestamp)
        `);
    this.db.run(`
            CREATE INDEX IF NOT EXISTS idx_endpoint_access_endpoint 
            ON endpoint_access_logs(endpoint, timestamp)
        `);
    this.db.run(`
            CREATE INDEX IF NOT EXISTS idx_endpoint_access_timestamp 
            ON endpoint_access_logs(timestamp)
        `);
    ...
    this.db.run(
      ...
      (err) => {
        if (err) {
          logger.error("Error creating recovery snapshot index:", err);
        }
      }
    );
    ...
    this.db.run(
      ...
      (err) => {
        if (err) {
          logger.error("Error creating heat scores index:", err);
        }
      }
    );
    ...
    this.db.run(
      ...
      (err) => {
        if (err) {
          logger.error("Error creating whitelist index:", err);
        }
      }
    );
    this.db.run(
  ...
  async addEnhancedLog(
    ...
    // Encrypt potentially sensitive text fields inside metadata
    ["content", "old_content", "new_content", "message", "reason"].forEach(
      (key) => {
        if (typeof safeMetadata[key] === "string" && safeMetadata[key].length) {
          safeMetadata[key] = encryptText(safeMetadata[key]);
        }
      }
    );
      ...
      this.db.get(
        ...
        (err, row) => {
          if (err) {
            ...
            resolve(Math.round(row?.avgScore || 0));
          }
        }
      );
    });
            ...
            this.db.run(
              ...
              (err) => {
                if (err) {
                  ...
                  resolve({ allowed: true, remaining: row.rate_limit - 1 });
                }
              }
            );
  ...
  async getUserAPIKey(discordUserId) {
    return new Promise((resolve, reject) => {
      this.db.get(
      ...
      this.db.run(
        ...
        function (err) {
          if (err) {
            ...
            resolve({ id: this.lastID, source, description });
          }
        }
      );
    });
  ...
  updateAutomodConfig(guildId, config) {
    return new Promise((resolve, reject) => {
      ...
      const query = `UPDATE automod_config 
    ...
    return new Promise((resolve, reject) => {
      this.db.run(
        ...
        [
          ...
          userId,
          violationType,
          messageContent ? encryptText(messageContent.substring(0, 200)) : "",
          actionTaken,
          Date.now(),
        ],
        (err) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.db.all(query, params, (err, rows) => {
        ...
        const safeRows = (rows || []).map((row) => {
          if (
            Object.prototype.hasOwnProperty.call(row, "message_content") &&
            row.message_content !== null &&
            row.message_content !== undefined
          ) {
            return {
              ...row,
              message_content: decryptText(row.message_content),
            };
          }
          return row;
        });
          ...
          if (row.metadata) {
            try {
              ...
              [
                ...
                if (typeof meta[key] === "string" && meta[key].length) {
                  meta[key] = decryptText(meta[key]);
                }
              });
      ...
      this.db.all(
        ...
        (testErr, testRows) => {
          if (
            ...
            this.db.all(
              `SELECT id, achievement_type, achievement_data FROM achievements ORDER BY id`,
              [],
              (err, rows) => {
                if (err) {
                  resolve([]);
            ...
            // Try new structure - but only select columns that definitely exist (no achievement_id)
            this.db.all(
