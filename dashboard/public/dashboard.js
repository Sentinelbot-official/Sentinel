...
function sanitizeForHtml(str, maxLength = 1000) {
  if (typeof str !== "string") {
    return "";
  }
  let sanitized = str.replace(/\0/g, "").replace(/[\x00-\x1F\x7F-\x9F]/g, "");
...
function loadPage(page) {
  // Pages that don't require a server selection
  if (page === "bulk") {
    loadBulkOperations();
    return;
  }
  ...
  if (page === "api-keys") {
    try {
      if (typeof loadAPIKeysPage === "function") {
        loadAPIKeysPage();
      } else {
        console.error(
          "loadAPIKeysPage function not found! Available functions:",
          Object.keys(window).filter(
            (k) => k.includes("API") || k.includes("api")
          )
        );
        ...
        if (contentArea) {
          contentArea.innerHTML = `
            ...
            <div style="padding: 20px;">
              <p>If this page doesn't load, please refresh the browser.</p>
            </div>
          `;
          // Try to call it after a short delay in case it's not loaded yet
          setTimeout(() => {
            if (typeof loadAPIKeysPage === "function") {
              loadAPIKeysPage();
            }
      ...
      alert(`Error loading API keys page: ${error.message}`);
    }
    return;
  }
    ...
    <div class="logs-table">
      ...
      ${logs
        ...
        <div class="log-row">
          ...
          <div><span class="action-badge action-${sanitizeForHtml(log.action, 50)}">${sanitizeForHtml(log.action.toUpperCase(), 50)}</span></div>
          <div><code>${sanitizeForHtml(log.user_id || "", 50)}</code></div>
          <div><code>${sanitizeForHtml(log.moderator_id || "", 50)}</code></div>
          <div>${sanitizeForHtml(log.reason || "None", 200)}</div>
        </div>
      `
        )
        .join("")}
            ...
            <div><span class="score-badge score-${
              log.threat_score >= 70
                ...
                : log.threat_score >= 40
                  ? "medium"
                  : "low"
            }">${sanitizeForHtml(String(log.threat_score || 0), 10)}</span></div>
            <div>${sanitizeForHtml(log.details || "No details", 200)}</div>
          </div>
        `
          )
          .join("")}
      ...
      ${logs
        .map((log) => {
          ...
          if (log.log_type === "message_update") {
            ...
            contentPreview =
              sanitizeForHtml(meta.content?.substring(0, 100) || "") ||
              sanitizeForHtml(meta.oldContent?.substring(0, 100) || "") ||
              "N/A";
          }
          ...
          <div class="log-row">
            <div>${sanitizeForHtml(new Date(log.timestamp).toLocaleString())}</div>
            <div><span class="action-badge action-${sanitizeForHtml(log.log_type, 50)}">${typeIcon} ${typeLabel}</span></div>
            <div>#${sanitizeForHtml(meta.channelName || "unknown", 100)} <code style="font-size:0.8em; opacity:0.7;">${sanitizeForHtml(meta.channelId || "", 50)}</code></div>
            <div><code>${sanitizeForHtml(meta.authorTag || log.user_id || "N/A", 100)}</code></div>
            <div><code>${log.moderator_id ? `<@${sanitizeForHtml(log.moderator_id, 50)}>` : "System"}</code></div>
  ...
  document.querySelectorAll(".nav-item").forEach((item) => {
    item.addEventListener("click", (e) => {
      ...
      if (pageName === "overview") {
        ...
        // API keys page doesn't require server selection
        loadAPIKeysPage();
      } else {
        loadPage(pageName);
      }
...
async function loadAPIKeysPage() {
  const contentArea = document.getElementById("contentArea");
  document.getElementById("currentPage").textContent = "API Keys";
  ...
  contentArea.innerHTML = `
    ...
    <div id="apiKeysContent" style="margin-top: 30px;">
      <div class="loading">Loading...</div>
    </div>
  `;

  try {
    await loadAPIKeysContent();
  } catch (error) {
    ...
    contentArea.innerHTML = `
      ...
      <div style="color: #ff6b6b; padding: 20px; background: rgba(255, 107, 107, 0.1); border-radius: 10px; margin-top: 20px;">
...
async function loadAPIKeysContent() {
  const contentArea = document.getElementById("apiKeysContent");

  try {
    // Fetch active API key and requests
    const [activeKeyRes, requestsRes] = await Promise.all([
      fetch("/api/apikey/active", { credentials: "include" }),
      fetch("/api/apikey/requests", { credentials: "include" }),
    ]);

    if (!activeKeyRes.ok || !requestsRes.ok) {
    ...
    const hasActiveKey = activeKeyData.success && activeKeyData.hasKey;
    const activeKey = activeKeyData.apiKey;
    const requests = requestsData.requests || [];

    // Find pending request
    const pendingRequest = requests.find(
      (r) => r.status === "pending" && r.expires_at > Date.now()
    );

    let html = "";

    // Active API Key Section
    if (hasActiveKey && activeKey) {
      html += `
        <div class="setting-card" style="margin-bottom: 30px;">
          <h2 style="margin-bottom: 15px;">✅ Active API Key</h2>
          <div style="background: rgba(0, 255, 136, 0.1); padding: 20px; border-radius: 10px; border: 2px solid rgba(0, 255, 136, 0.3);">
            <div style="margin-bottom: 15px;">
              <strong>Purpose:</strong> ${activeKey.purpose || "N/A"}
            </div>
            <div style="margin-bottom: 15px;">
              <strong>Partial Key:</strong> <code style="background: rgba(0,0,0,0.3); padding: 5px 10px; border-radius: 5px;">${activeKey.partial_key || "N/A"}</code>
            </div>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-top: 15px;">
              <div>
                <strong>Created:</strong><br>
                <span style="opacity: 0.8;">${new Date(activeKey.created_at).toLocaleString()}</span>
              </div>
              <div>
                <strong>Last Used:</strong><br>
                <span style="opacity: 0.8;">${activeKey.last_used ? new Date(activeKey.last_used).toLocaleString() : "Never"}</span>
              </div>
              <div>
                <strong>Requests Today:</strong><br>
                <span style="opacity: 0.8;">${activeKey.requests_today || 0} / ${activeKey.rate_limit || "Unlimited"}</span>
              </div>
              <div>
                <strong>Total Requests:</strong><br>
                <span style="opacity: 0.8;">${activeKey.total_requests || 0}</span>
              </div>
            </div>
            <div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid rgba(255,255,255,0.1);">
              <strong>Status:</strong> <span style="color: ${activeKey.is_active ? "#00ff88" : "#ff6b6b"}">${activeKey.is_active ? "Active" : "Inactive"}</span>
            </div>
          </div>
        </div>
      `;
      ...
      html += `
        <div class="setting-card" style="margin-bottom: 30px; border: 2px dashed rgba(255,255,255,0.2);">
          <h2 style="margin-bottom: 15px;">No Active API Key</h2>
          <p style="opacity: 0.8; margin-bottom: 20px;">You don't have an active API key. Request one below to get started.</p>
        </div>
      `;
    }
    ...
    if (pendingRequest) {
      const expiresIn = Math.max(
        0,
        Math.floor((pendingRequest.expires_at - Date.now()) / 1000 / 60)
      );
      html += `
        <div class="setting-card" style="margin-bottom: 30px; border: 2px solid #ffd700;">
    ...
    // Request New API Key Section
    if (!hasActiveKey && !pendingRequest) {
      html += `
        <div class="setting-card" style="margin-bottom: 30px;">
          <h2 style="margin-bottom: 15px;">📝 Request New API Key</h2>
          <div style="background: rgba(102, 126, 234, 0.1); padding: 20px; border-radius: 10px;">
            <form id="apiKeyRequestForm" onsubmit="requestAPIKey(event)">
              <div style="margin-bottom: 20px;">
                <label style="display: block; margin-bottom: 8px; font-weight: bold;">Purpose *</label>
                <input 
                  type="text" 
                  id="purposeInput" 
                  placeholder="e.g., Automated moderation for my server" 
                  required 
                  minlength="10"
                  style="width: 100%; padding: 12px; border-radius: 8px; border: 2px solid #667eea; background: #1a202c; color: white; font-size: 14px;"
                />
                <small style="opacity: 0.7; display: block; margin-top: 5px;">Minimum 10 characters. Describe what you'll use the API key for.</small>
              </div>
              <div style="margin-bottom: 20px;">
                <label style="display: block; margin-bottom: 8px; font-weight: bold;">Notes (Optional)</label>
                <textarea 
                  id="notesInput" 
                  placeholder="Additional details about your use case..." 
                  rows="3"
                  style="width: 100%; padding: 12px; border-radius: 8px; border: 2px solid #667eea; background: #1a202c; color: white; font-size: 14px; resize: vertical;"
                ></textarea>
              </div>
              <button type="submit" class="btn" style="width: 100%; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
                📝 Request API Key
              </button>
            </form>
          </div>
    ...
    if (requests.length > 0) {
      html += `
        <div class="setting-card">
          <h2 style="margin-bottom: 15px;">📜 Request History</h2>
                ...
                ${requests
                  .map((req) => {
                    ...
                    <tr style="border-bottom: 1px solid rgba(255,255,255,0.05);">
                      <td style="padding: 12px;">${req.purpose || "N/A"}</td>
                      <td style="padding: 12px;">
                        <span style="color: ${statusColors[req.status] || "#fff"}">
                          ${statusLabels[req.status] || req.status}
                        </span>
                      </td>
                      <td style="padding: 12px; opacity: 0.8;">${new Date(req.created_at).toLocaleString()}</td>
                      <td style="padding: 12px; opacity: 0.8;">${req.expires_at ? new Date(req.expires_at).toLocaleString() : "N/A"}</td>
                    </tr>
                  `;
                  })
                  .join("")}
    ...
    contentArea.innerHTML = `
      <div style="color: #ff6b6b; padding: 20px; background: rgba(255, 107, 107, 0.1); border-radius: 10px;">
        ❌ Failed to load API keys: ${error.message}
      </div>
    `;
  }
...
async function requestAPIKey(event) {
  ...
  try {
    const response = await fetch("/api/apikey/request", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (!response.ok) {
      // Check if user needs to join the server
      if (data.requiresServerJoin && data.oauthUrl) {
        const shouldJoin = confirm(
          "You must be in our Discord server to request an API key.\n\nWould you like to join our server now?"
        );
        if (shouldJoin) {
          ...
          alert(
            "You must be in our Discord server to request an API key. Please join our server and try again."
          );
          return;
        }
      }
      throw new Error(data.error || "Failed to request API key");
    }

    alert("✅ API key request created! Please confirm it within 15 minutes.");
    await loadAPIKeysContent();
  } catch (error) {
    alert(`❌ Error: ${error.message}`);
  }
...
async function confirmAPIKeyRequest(requestId) {
  if (
    !confirm(
      "Are you sure you want to confirm and generate this API key? This action cannot be undone."
    )
  ) {
    return;
  }
  ...
  try {
    const response = await fetch("/api/apikey/confirm", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (!response.ok) {
      throw new Error(data.error || "Failed to confirm API key");
    }

    // Show API key in a secure way
    const apiKey = data.apiKey;
    const message = `✅ API key generated successfully!\n\n⚠️ IMPORTANT: Copy this key now - it won't be shown again!\n\n${apiKey}\n\n${data.warning || ""}`;

    // Create a modal to show the key
    const modal = document.createElement("div");
    modal.style.cssText = `
      position: fixed;
      top: 0;
    ...
    `;
    modal.innerHTML = `
      <div style="background: #1a202c; padding: 30px; border-radius: 15px; max-width: 500px; width: 90%; border: 2px solid #667eea;">
        <h2 style="margin-bottom: 15px; color: #00ff88;">✅ API Key Generated</h2>
        <p style="margin-bottom: 20px; color: #ffd700;">⚠️ Copy this key now - it won't be shown again!</p>
        <div style="background: rgba(0,0,0,0.5); padding: 15px; border-radius: 8px; margin-bottom: 20px; word-break: break-all; font-family: monospace; color: #00ff88;">
          ${apiKey}
        </div>
        <button onclick="this.parentElement.parentElement.remove(); loadAPIKeysContent();" style="width: 100%; padding: 12px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; border-radius: 8px; font-weight: bold; cursor: pointer;">
          I've Copied It
        </button>
      </div>
    `;
    ...
    const keyElement = modal.querySelector("div[style*='word-break']");
    if (keyElement) {
...
async function declineAPIKeyRequest(requestId) {
  if (!confirm("Are you sure you want to decline this API key request?")) {
    return;
  }

  try {
    const response = await fetch("/api/apikey/decline", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (!response.ok) {
      throw new Error(data.error || "Failed to decline API key request");
    }

    alert("✅ API key request declined.");
    await loadAPIKeysContent();
  } catch (error) {
    alert(`❌ Error: ${error.message}`);
  }
