  ...
  async applyPreset(interaction) {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setColor(0x00ff00)
        .setFooter({
          text: "Need help? discord.gg/9vQzqBVMNX | Your server is now protected!",
        })
        .setTimestamp();
    ...
    try {
      ...
      if (config) {
        if (config.mod_log_channel) {
          ...
          incomplete.push("❌ Mod log channel not set");
        }

        if (config.anti_raid_enabled) {
          completed.push("✅ Anti-raid enabled");
        } else {
          incomplete.push("❌ Anti-raid disabled");
        }

        if (config.anti_nuke_enabled) {
          completed.push("✅ Anti-nuke enabled");
        } else {
          incomplete.push("❌ Anti-nuke disabled");
        }

        if (config.auto_mod_enabled) {
          completed.push("✅ Auto-moderation enabled");
        } else {
          incomplete.push("❌ Auto-moderation disabled");
        }
      } else {
        incomplete.push(
      ...
      const embed = new EmbedBuilder()
        ...
        .setFooter({
          text: "Need help? discord.gg/9vQzqBVMNX | Use /setup preset or /setup wizard",
        })
        .setTimestamp();
